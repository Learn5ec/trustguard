...
function normalizeSpdxId(raw: string | null | undefined): string | undefined {
  ...
  if (!trimmed || trimmed === "UNKNOWN" || trimmed === "unknown")
    return undefined;
 ...
 */
export function canonicalizeGithubUrl(
...
async function lookupNpm(
  ...
  if (!resp.ok)
    throw new Error(`npm registry returned ${resp.status} for ${name}`);
  ...
  const versionData =
    version && version !== "latest" && data.versions
      ? data.versions[version]
      : data.versions?.[data["dist-tags"]?.latest];

  const repoUrl = versionData?.repository?.url || data.repository?.url || null;

  const isDeprecated = !!(versionData?.deprecated || data.deprecated);
  const deprecationMessage =
    versionData?.deprecated || data.deprecated || undefined;
  ...
  return {
    ...
    confirmedVersion:
      version !== "latest" ? version : data["dist-tags"]?.latest,
    latestVersion: data["dist-tags"]?.latest,
    isDeprecated,
    deprecationMessage:
      typeof deprecationMessage === "string" ? deprecationMessage : undefined,
...
async function lookupPypi(
  ...
  const endpoint =
    version && version !== "latest"
      ? `${REGISTRY_BASES.pypi}${encodeURIComponent(name)}/${encodeURIComponent(
          version
        )}/json`
  ...
  if (!resp.ok) {
    if (resp.status === 404 && version) {
      throw new Error(
        `Version ${version} not found in PyPI for ${name} (checked: ${endpoint})`
      );
    }
    throw new Error(
      `PyPI returned ${resp.status} for ${name} (checked: ${endpoint})`
    );
  }
  const data = await resp.json();
  ...
  const candidates: (string | null | undefined)[] = [
    ...
    info.project_urls?.["Issue Tracker"],
    info.home_page,
  ];
  ...
  const isDeprecated = classifiers.some(
    (c) =>
      c.includes("Development Status :: 7 - Inactive") ||
      (c.includes("Development Status :: 1 - Planning") &&
  ...
  if (requestedVer && data.releases?.[requestedVer]?.[0]?.upload_time) {
    try {
      ...
      latestVersionPublishedAt = new Date(
        data.releases[requestedVer][0].upload_time + "Z"
      );
    } catch {
      /* ignore */
    }
...
async function lookupCratesIo(
  ...
  if (!resp.ok)
    throw new Error(`crates.io returned ${resp.status} for ${name}`);
  const data = await resp.json();
  ...
  if (version && version !== "latest") {
    ...
    const found = versions.find(
      (v: any) => v.num === version || v.num === `v${version}`
    );
    if (!found)
      throw new Error(`Version ${version} not found in crates.io for ${name}`);
  }
  ...
  return {
    ...
    resolvedRegistryUrl: url,
    confirmedVersion:
      version !== "latest" ? version : data.crate?.newest_version,
    latestVersion: data.crate?.newest_version,
    description: data.crate?.description,
  };
}

async function lookupPubDev(
  name: string,
  _version?: string
): Promise<RegistryLookupResult> {
...
async function lookupNuGet(name: string): Promise<RegistryLookupResult> {
  const url = `${REGISTRY_BASES.nuget}${encodeURIComponent(
    name.toLowerCase()
  )}/index.json`;
  const resp = await fetch(url, { headers: { Accept: "application/json" } });
...
async function lookupHex(name: string): Promise<RegistryLookupResult> {
  ...
  const githubLink =
    data.meta?.links?.GitHub || data.meta?.links?.github || null;
  return {
...
async function lookupPackagist(name: string): Promise<RegistryLookupResult> {
  ...
  const resp = await fetch(url, { headers: { Accept: "application/json" } });
  if (!resp.ok)
    throw new Error(`Packagist returned ${resp.status} for ${name}`);
  const data = await resp.json();

  // Get latest version data
  const versions: Record<string, any> = data.package?.versions || {};
  const latestKey = Object.keys(versions).find(
    (v) => !v.includes("dev") && !v.startsWith("9999")
  );
...
function lookupGo(modulePath: string): RegistryLookupResult {
  ...
  const githubUrl = canonicalizeGithubUrl(
    modulePath.startsWith("github.com/") ? `https://${modulePath}` : null
  );
...
async function lookupMaven(
  ...
  if (parts.length < 2)
    throw new Error(
      `Maven artifact must be in groupId:artifactId format, got: ${groupArtifact}`
    );

  const [groupId, artifactId] = parts;
  const searchUrl = `${MAVEN_SEARCH_BASE}?q=g:${encodeURIComponent(
    groupId
  ...
  if (!doc)
    throw new Error(`Package ${groupArtifact} not found in Maven Central`);
  ...
  if (latestVersion) {
    try {
      const pomUrl = `${MAVEN_REPO_BASE}${groupId.replace(
        /\./g,
        "/"
...
export async function lookupRegistry(
  ...
  switch (ecosystem) {
    case "npm":
      return lookupNpm(packageName, ver);
    case "pypi":
    case "uv":
    case "pip":
    case "pipx":
      return lookupPypi(packageName, ver);
    case "rust":
      return lookupCratesIo(packageName, ver);
    case "pub":
      return lookupPubDev(packageName, ver);
    case "ruby":
      return lookupRubyGems(packageName);
    case "nuget":
      return lookupNuGet(packageName);
    case "hex":
      return lookupHex(packageName);
    case "packagist":
      return lookupPackagist(packageName);
    case "go":
      return lookupGo(packageName);
    case "maven":
      return lookupMaven(packageName);
    case "conda":
      // Conda doesn't have a clean JSON API — return null GitHub URL
      return {
        githubUrl: null,
        resolvedVia: "registry_lookup",
        resolvedRegistryUrl: `https://anaconda.org/search?q=${encodeURIComponent(
          packageName
        )}`,
      };
    default:
